...
- **Open decision, unchanged since `homepage.md`'s flag**: the live site (homepage *and* the new `/contact` page) shows `leon@primeturf.co.za` in the footer, not `social@primeturf.co.za`. This was already flagged as unresolved — it's now shipped to production both places with `leon@`. Needs an explicit call on which inbox is actually monitored before more pages go live with either address baked in.
